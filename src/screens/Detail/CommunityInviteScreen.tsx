...
export const CommunityInviteScreen: React.FC = () => {
  ...
  const handleShare = async () => {
    try {
      await Share.share({
        title: `Rejoins ${communityName} sur GoFolyX !`,
        message:
          `🔥 Rejoins notre communauté "${communityName}" sur GoFolyX !\n\n` +
          `👉 Code d'invitation : *${code}*\n\n` +
          `Télécharge GoFolyX et entre ce code pour rejoindre le groupe.\n` +
          `https://gofolyx.app/join/${code}`,
      });
    } catch { }
  };
      ...
      <View style={{ flex: 1, padding: 24, gap: 24, justifyContent: 'center' }}>
        ...
        <View style={[st.infoBox, { backgroundColor: '#7B3FF210', borderColor: '#7B3FF230' }]}>
          ...
          <Text style={{ color: '#7B3FF2', fontSize: 13, flex: 1, lineHeight: 19 }}>
            Partage ce code avec qui tu veux inviter. Il suffira de le saisir dans GoFolyX pour rejoindre ta communauté.
          </Text>
        </View>
