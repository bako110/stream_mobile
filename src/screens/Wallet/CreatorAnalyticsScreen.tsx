  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const [ov, content] = await Promise.all([
        ...
        analyticsService.listContent({ contentType: ct, period, sort: 'views', limit: 10 }),
      ]);
      setOverview(ov);
      ...
      >
        ...
        {loading ? (
          ...
          <>
            ...
            {/* Contenus les plus vus — indépendant des vues de la PÉRIODE : un
                contenu sans vue récente doit quand même apparaître dans la liste
                complète (via "Voir tout"), pas disparaître de l'écran. */}
            <View style={[s.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              <View style={s.cardHeaderRow}>
                <Text style={[s.cardTitle, { color: colors.textPrimary }]}>Contenus les plus vus</Text>
                <TouchableOpacity
                  onPress={() => (nav as any).navigate('ContentAnalyticsList', {
                    period, contentType: contentType === 'all' ? undefined : contentType,
                  })}
              ...
              {contentItems.length > 0 ? (
                ...
                <Text style={[s.emptyInlineText, { color: colors.textTertiary }]}>
...
const s = StyleSheet.create({
  ...
  seeAllText:     { fontSize: 12.5, fontWeight: '700' },
